<template>
	<TransitionGroup name="list" tag="ul">
			<div class="mb-2.5 p-2.5" v-for="(item, index) in data" :key="index" style="border-radius: 0.75rem"
				...
				<div class="moment_info">
					...
					<!-- 弹出选项 -->
					<n-space :size="2">
						<n-tag v-for="(n, index) in item.tags" :key="index" :bordered="false" round size="small"
							:color="{ color: n.color, textColor: n.textColor }">
							{{ $t(n.name) }}
						</n-tag>
					</n-space>
				</div>
